...
<head>
    ...
    <script>
        $(function(){
            $('#pagination').bootstrapPaginator({
                ...
                pageUrl:function(type,page, current){
                    return '<%=request.getContextPath()%>/backend/productType/findAll?pageNum='+page;
                },
        ...
        //显示商品类型 点击修改按钮后，将指定id的产品类型数据填充到模态框中
        function showProductType(id){
            $.post(
                '<%=request.getContextPath()%>/backend/productType/findById',
                {'id':id},
                function(result){
                    if(result.status==1){
        ...
        function deleteProductType(){
            $.get(
                '<%=request.getContextPath()%>/backend/productType/removeById',
        ...
        function modifyStatus(id,btn){
            $.get(
                '<%=request.getContextPath()%>/backend/productType/modifyStatus',
                {'id':id},
                function(result){
                    if(result.status==1){
                        //局部刷新
...
<div class="panel panel-default" id="userSet">
    ...
    <div class="panel-body">
        ...
        <!--点击添加商品类型这一个按钮会弹出一个输入框 why todo -->
